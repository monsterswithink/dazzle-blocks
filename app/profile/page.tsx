...
export default function ProfilePage() {
    ...
    const fetchProfileAndEnrich = async () => {
      if (status === "authenticated") {
        try {
          ...
          toast.error("Failed to load or enrich profile", { description: err.message })
        } finally {
          setLoading(false)
        }
  ...
  const handleCreateOrUpdateProfile = async (profileData: Profile) => {
    ...
    try {
      ...
      setError(err.message)
      toast.error(`Failed to ${profileData.id ? "update" : "create"} profile`, { description: err.message })
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleDeleteProfile = async () => {
    ...
    setError(null)
    try {
      const response = await fetch(`/api/profile?id=${profile.id}`, { method: "DELETE" })
          ...
          {profile ? (
            <div className="space-y-4">
              <div>
                ...
                <Input id="name" name="name" value={profile.personal.name} onChange={handleChange} disabled={!isEditing} />
              </div>
              <div>
                <Label htmlFor="title">Title</Label>
                <Input id="title" name="title" value={profile.personal.title} onChange={handleChange} disabled={!isEditing} />
              </div>
              <div>
                <Label htmlFor="email">Email</Label>
                <Input id="email" name="email" type="email" value={profile.personal.email} onChange={handleChange} disabled={!isEditing} />
              </div>
              <div>
                <Label htmlFor="phone">Phone</Label>
                <Input id="phone" name="phone" type="tel" value={profile.personal.phone} onChange={handleChange} disabled={!isEditing} />
              </div>
              <div>
                <Label htmlFor="linkedin">LinkedIn</Label>
                <Input id="linkedin" name="linkedin" type="url" value={profile.personal.linkedin} onChange={handleChange} disabled={!isEditing} />
              </div>
              <div>
                <Label htmlFor="github">GitHub</Label>
                <Input id="github" name="github" type="url" value={profile.personal.github} onChange={handleChange} disabled={!isEditing} />
              </div>
              {isEditing && (
